...
const DevicesPage = () => {
  ...
  const filteredDevices = devices.filter((device) => {
    ...
    const matchesWard =
      filterWardId === "all" || device.wardId === filterWardId;
  ...
  // Tại trung tâm: phân phát xuống phường dựa trên wardId
  const assignedDevices = filteredDevices.filter((d) => !!d.wardId);
  const unassignedDevices = filteredDevices.filter((d) => !d.wardId);
    ...
    try {
      ...
      await Promise.all(
        Array.from({ length: quantity }).map((_, i) => {
          const nameWithIndex =
            quantity > 1 ? `${newDevice.name} #${i + 1}` : newDevice.name;
...
const handleDeleteDevice = async (deviceId: string) => {
  ...
  return (
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        <div>
          ...
          <p className="text-muted-foreground">
            Quản lý tất cả thiết bị IT trong hệ thống
          </p>
        </div>
              ...
              <div className="grid grid-cols-2 gap-4 py-4">
                ...
                <div className="space-y-2">
                  ...
                  <Input
                    ...
                    value={addQuantity}
                    onChange={(e) =>
                      setAddQuantity(parseInt(e.target.value || "1") || 1)
                    }
                  />
                </div>
                ...
                {newDevice.type && (
                  <SpecEditor
                    ...
                    onChange={(next) =>
                      setNewDevice({ ...newDevice, specifications: next })
                    }
                  />
                )}
                ...
                <div className="space-y-2">
                  ...
                  <Input
                    ...
                    type="date"
                    value={
                      newDevice.installationDate.toISOString().split("T")[0]
                    }
                    onChange={(e) =>
                      setNewDevice({
                        ...newDevice,
                        installationDate: new Date(e.target.value),
                      })
                    }
                  />
                </div>
            ...
            {editingDevice && (
              <div className="grid grid-cols-2 gap-4 py-4">
                ...
                <SpecEditor
                  ...
                  onChange={(next) =>
                    setEditingDevice({ ...editingDevice, specifications: next })
                  }
                />
              </div>
            )}
                ...
                onClick={async () => {
                  ...
                  try {
                    ...
                    toast({
                      title: "Lỗi cập nhật",
                      description:
                        error.message || "Không thể cập nhật thiết bị",
                      variant: "destructive",
                    });
                  }
                }}
      ...
      {/* Devices Table with assignment tabs */}
      <Card>
        <CardHeader>
          <div className="flex items-center justify-between">
            <CardTitle>
              Danh sách thiết bị ({displayedDevices.length})
            </CardTitle>
            <Tabs
              value={assignmentTab}
              onValueChange={(v) => setAssignmentTab(v as any)}
            >
              <TabsList>
                <TabsTrigger value="all">Tất cả</TabsTrigger>
                <TabsTrigger value="assigned">Đã phân phát</TabsTrigger>
                <TabsTrigger value="unassigned">Chưa phân phát</TabsTrigger>
              </TabsList>
            </Tabs>
          </div>
        </CardHeader>
            ...
            <TableBody>
              {displayedDevices.map((device) => (
                <TableRow key={device.id}>
                  ...
                  <TableCell>
                    <div className="text-sm">
                    <p>{wardNameMap[device.wardId] || device.wardName || "—"}</p>
                    </div>
                  </TableCell>
                  <TableCell>
                    <div className="text-sm">
                      <p>{device.assignedToName || "—"}</p>
                    </div>
                  </TableCell>
                  <TableCell>
                    <Badge
                      ...
                      <DialogContent className="max-w-2xl">
                        ...
                        <div className="grid grid-cols-2 gap-4 text-sm">
                          <div>
                            ...
                            <p><strong>Phường:</strong> {wardNameMap[device.wardId] || device.wardName || "—"}</p>
                            <p>
                          ...
                          <div className="mt-4">
                            ...
                            <ul className="list-disc list-inside text-sm">
                              {getFieldsForType(device.type || "other")
                                .filter(
                                  (k) => k !== "vendor" && k !== "description"
                                )
                                .map((k) => {
                                  const meta = FIELD_META[k];
                                  const rawVal = (device.specifications as any)?.[
                                    k
                                  ];
                                  if (!meta || rawVal === undefined || rawVal === "")
                                    return null;
                                  const displayVal =
  k === 'license'
